jQuery(document).ready(function ($) {
    ...
    function open_popup() {
        ...
        if (wcacn_cp_localize.closeafter != '' && wcacn_cp_localize.closeafter != 0) {
            setTimeout(function () {
    ...
    function show_notice(notice_type, notice) {
        $('.wcacn-cp-notice').html(notice).attr('class', 'wcacn-cp-notice').addClass('wcacn-cp-nt-' + notice_type);
    ...
    function add_to_cart(atc_btn, form_data) {
        ...
        $.ajax({
            url: wcacn_cp_localize.wc_ajax_url.toString().replace('%%endpoint%%', 'wcacn_cp_add_to_cart'),
            type: 'POST',
            ...
            success: function (response) {
                ...
                if (response.fragments) {
                    ...
                    atc_btn.append('<span class="wcacn-cp-icon-check wcacn-cp-added"></span>');
                }
    ...
    $(document).on('submit', 'form.cart', function (e) {
        ...
        $('.wcacn-cp-added', atc_btn).remove();
        atc_btn.append('<span class="wcacn-cp-icon-spinner wcacn-cp-adding" aria-hidden="true"></span>');
    ...
    function update_cart(cart_key, new_qty) {
        block_popup();
        $.ajax({
            url: wcacn_cp_localize.wc_ajax_url.toString().replace('%%endpoint%%', 'wcacn_cp_update_cart'),
            type: 'POST',
            data: {
                cart_key: cart_key,
                new_qty: new_qty
    ...
    $(document).on('change', '.wcacn-cp-qty', function (e) {
        ...
        if (new_qty === 0) {
            _this.parents('.wcacn-cp-pdetails').find('.wcacn-cp-remove-pd').trigger('click');
            return;
        }
    ...
    $(document).on('click', '.wcacn-chng', function () {
        ...
        if (_this.hasClass('wcacn-plus')) {
            ...
            var new_qty = input_qty - step;
            if (new_qty === 0) {
                _this.parents('.wcacn-cp-pdetails').find('.wcacn-cp-remove .wcacn-icon').trigger('click');
                return;
            }
            else if (new_qty < min_value) {
                return;
